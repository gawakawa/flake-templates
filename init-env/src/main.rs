...
fn flake_templates_ref() -> String {
    std::env::var("INIT_ENV_FLAKE_REF").unwrap_or_else(|_| DEFAULT_FLAKE_TEMPLATES_REF.to_string())
}
...
/// the flake templates ref, sorted alphabetically by name.
fn fetch_templates() -> io::Result<Vec<(String, String)>> {
    let flake_ref = flake_templates_ref();
    let json = capture(
        "nix",
        &[
            "eval",
            "--json",
            &format!("{flake_ref}#templates"),
            "--apply",
...
fn apply_template(template: &str, dir: &Path) -> io::Result<()> {
    ...
    let template_ref = format!("{}#{template}", flake_templates_ref());
